exports.run = async (client, message, args, level) => {
    try {
        ...
        const items = ['weddingring', 'ring', 'rings', 'food', 'petfood', 'seed', 'seeds', 'weed', 'drugs', 'cocaine', 'blow', 'acid']
        ...
        if (args[0] === 'weed' || args[0] === 'drugs') {
            const weeds = args[0] > 1 ? 'to get high' : 'weed'
            buyItem(money, 150, args[1], weeds)
        }
        if (args[0] === 'acid' || args[0] === 'acid') {
            const acids = args[0] > 1 ? 'to get high' : 'acid'
            buyItem(money, 200, args[1], acids)
        }
        if (args[0] === 'cocaine' || args[0] === 'blow') {
            const cocaines = args[0] > 1 ? 'to get high' : 'cocaine'
            buyItem(money, 1500, args[1], cocaines)
        }
    } catch (err) {
